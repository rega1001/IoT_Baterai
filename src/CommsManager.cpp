...
void CommsManager::loop() {
  ...
  // 2. Cek Koneksi WiFi
  if (!WiFi.isConnected() && (millis() - _lastSetupTime > 10000)) {
    _lastSetupTime = millis();
    ...
    if (_serialBT.available()) {
      ...
      if (c == '\n') {
        ...
        if (_pos < 49) {
          ...
          else if (_isWaitingForPass) _inputPASS[_pos] = c;
          else _siteName[_pos] = c;
          _pos++;
        }
      }
    }
...
// ==========================================
void CommsManager::getSiteName(char* buffer, size_t maxLen) {
  _pref.begin("config", true);
  if (!_pref.isKey("site_name")) {
      buffer[0] = '\0'; 
  } else {
      _pref.getString("site_name", buffer, maxLen);
  }
  _pref.end();
}